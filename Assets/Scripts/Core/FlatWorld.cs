...
public class FlatWorld {
    ...
    private void ResolveCollisionWithRotationAndFriction(in FlatManifold contact) {
        ...
        for (var i = 0; i < contactCount; i++) {
            ...
            if (Utils.NearlyEqual(tangent, Vector3.zero)) continue;
            ...
            bodyA.AngularVelocity -= Utils.Cross(rA, frictionImpulse) * bodyA.invInertia;
            bodyB.LinearVelocity += frictionImpulse * bodyB.invMass;
            bodyB.AngularVelocity += Utils.Cross(rB, frictionImpulse) * bodyB.invInertia;
        }
    }
}
